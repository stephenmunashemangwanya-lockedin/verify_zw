...
The backend is available at `http://localhost:3000`, frontend at `http://localhost:5173`, and local RPC at loopback port 8545. The one-shot deployment job reads the runtime chain ID and checks `eth_getCode` before trusting metadata in the named volume. Active metadata is reused; missing, wrong-chain, or stale local metadata is replaced by a verified local deployment. The backend mounts that metadata read-only, verifies the runtime chain and bytecode, and sets its in-process `CONTRACT_ADDRESS` before startup. No manual address copy is required.
