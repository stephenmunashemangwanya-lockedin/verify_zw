...
const audit = (req, action, target, details = {}) =>
  createAuditLog({
    ...
    institutionId:
      target?.institution_id || req.user.institutionId,
    action,
    entityType: "user",
    entityId: target?.id,
    details,
...
const targetForActor = async (req, id) => {
  ...
  if (
    ...
    return {
      error: [
        403,
        "You cannot manage a user from another institution.",
      ],
    };
  }
...
const validateIdentity = ({ fullName, email }) => {
  ...
  if (!EMAIL.test(String(email || "").toLowerCase())) {
...
const list = async (req, res) => {
  ...
  const search = normaliseSearchTerm(
    req.query.search
  );
  ...
  if (
    ...
    return respond(
      res,
      403,
      "You cannot query users from another institution."
    );
  }
  ...
  const result = await listUsers({
    ...
    status: statusBoolean(req.query.status),
...
const getOne = async (req, res) => {
  const result = await targetForActor(
    req,
    req.params.id
  );
  ...
  return res.json({
    success: true,
    user: safeUser(
      await findSafeUserById(result.target.id)
    ),
  });
};
...
const create = async (req, res) => {
  ...
  if (identityError) {
    return respond(res, 400, identityError);
  }
  ...
  if (
    ...
    return respond(
      res,
      403,
      "Your institution is inactive or unassigned."
    );
  }
  ...
  if (
    req.user.role ===
      ROLES.INSTITUTION_ADMIN &&
    !INSTITUTION_MANAGED_ROLES.includes(role)
  ) {
    return respond(
      res,
      403,
      "Institution administrators may create only issuer, verifier, or student accounts."
    );
  }
  ...
  if (
    ...
    return respond(
      res,
      400,
      "A valid institution is required for this role."
    );
  }
  ...
  if (effectiveInstitution) {
    ...
    if (!institution.status) {
      return respond(
        res,
        422,
        "Users cannot be assigned to an inactive institution."
      );
    }
  }
  ...
  const response = {
    success: true,
    message:
      "User created with password setup required.",
    user: safeUser(
      await findSafeUserById(user.id)
    ),
  };

  if (process.env.NODE_ENV !== "production") {
    response.developmentPasswordSetupToken =
      reset.raw;
  }
...
const update = async (req, res) => {
  ...
  if (
    ...
    return respond(
      res,
      400,
      "Only fullName and email may be updated here."
    );
  }
  ...
  const before = {
    fullName: result.target.full_name,
    email: result.target.email,
  };

  const user = await updateUserFields(
    result.target.id,
    fields
  );
...
const changeStatus = async (req, res) => {
  ...
  if (
    !req.body.isActive &&
    result.target.id === req.user.userId
  ) {
    return respond(
      res,
      409,
...
const changeRole = async (req, res) => {
  ...
  if (
    req.user.role !== ROLES.SUPER_ADMIN &&
    !INSTITUTION_MANAGED_ROLES.includes(
      role
    )
  ) {
    return respond(
  ...
  if (
    ...
    return respond(
      res,
      422,
      "An institution must be assigned before applying this role."
    );
  }
  ...
  if (role !== ROLES.SUPER_ADMIN) {
    ...
    if (!institution?.status) {
      return respond(
        res,
        422,
        "The assigned institution is inactive."
      );
    }
  }

  const oldRole = result.target.role;

  const user =
    await updateUserRoleAndInstitution(
      result.target.id,
      role,
      institutionId
    );
...
const assignInstitution = async (
  ...
  if (
    ...
    return respond(
      res,
      403,
      "Only a super administrator may reassign institutions."
    );
  }
  ...
  if (
    ...
    return respond(
      res,
      409,
      "A global super administrator cannot be assigned to an institution."
    );
  }
  ...
  const user = await updateUserInstitution(
    result.target.id,
    institutionId
  );
  ...
  await audit(
    req,
    "USER_INSTITUTION_REASSIGNED",
    user,
    {
      oldInstitutionId,
      newInstitutionId: institutionId,
    }
  );
...
const requirePasswordChange = async (
  ...
  const user =
    await requireUserPasswordChange(
      result.target.id
    );
  ...
  return res.json({
    success: true,
    message:
      "Password change required. Existing sessions have been invalidated.",
    user: safeUser(user),
  });
};
...
const resetPassword = async (
  ...
  return res.json({
    success: true,
    message:
      "Password reset instructions have been requested through the configured delivery channel.",
  });
};
