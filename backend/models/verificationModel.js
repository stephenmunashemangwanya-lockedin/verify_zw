...
const publicCredentialSelect = `
  ...
  INNER JOIN students
    ON students.id = credentials.student_id

  INNER JOIN institutions
    ON institutions.id = credentials.institution_id
`;
...
const listVerificationLogs = async ({
  ...
  limit = 20,
  offset = 0,
  sortBy = "verification_time",
  sortOrder = "DESC",
} = {}) => {
  const fields = {
    verification_time:
      "verification_logs.verification_time",
    result:
      "verification_logs.result",
    verification_method:
      "verification_logs.verification_method",
  };
  ...
  if (credentialId) {
    add(
      "verification_logs.credential_id = ?",
      credentialId
    );
  }
  ...
  if (dateTo) {
    add(
      "verification_logs.verification_time < (?::date + INTERVAL '1 day')",
      dateTo
    );
  }
  ...
  const result = await pool.query(
    `SELECT
       ...
       verification_logs.result_code,
       verification_logs.verification_method,
       verification_logs.uploaded_hash,
       verification_logs.verification_time
     ${joins}
     ${clause}
     ORDER BY
       ${column} ${order},
       verification_logs.id ASC
